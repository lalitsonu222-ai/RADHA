...
import { ThemeType, CountingMode, JaapState } from './types';
import { THEMES, MALA_TARGET, BELL_SOUND_URL } from './constants';
import SpiritualQuote from './components/SpiritualQuote';

// Tap sound for every jaap (subtle spiritual chime)
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    tapAudioRef.current.volume = 0.3; // Make tap sound subtle
  }, []);
  ...
  const handleIncrement = useCallback(() => {
    // Haptic feedback for every bead
  ...
  return (
    <div 
      className="min-h-screen flex flex-col items-center p-4 transition-colors duration-700 select-none"
    ...
    >
      ...
      <header className="w-full max-w-md flex justify-between items-center py-6">
        ...
        <div className="flex gap-2">
          <button 
            ...
            style={{ backgroundColor: currentTheme.card, color: soundEnabled ? currentTheme.primary : '#999' }}
          ...
          <button 
            onClick={cycleTheme}
            className="p-3 rounded-full shadow-lg transition-transform active:scale-95"
            style={{ backgroundColor: currentTheme.card, color: currentTheme.primary }}
            title="Switch Theme"
          >
            <ThemeIcon />
          </button>
          <button 
            ...
            className="p-3 rounded-full shadow-lg transition-transform active:scale-95"
            style={{ backgroundColor: currentTheme.card, color: currentTheme.accent }}
            title="Reset Counter"
          >
            <ResetIcon />
          </button>
      ...
      <div className="w-full max-w-md flex justify-center mb-6">
        <div className="p-1 rounded-2xl flex gap-1 shadow-inner" style={{ backgroundColor: 'rgba(0,0,0,0.05)' }}>
          <button 
            onClick={() => setMode(CountingMode.MALA)}
            className={`px-6 py-2 rounded-xl text-xs font-bold transition-all ${mode === CountingMode.MALA ? 'shadow-sm' : 'opacity-40'}`}
            style={{ 
              backgroundColor: mode === CountingMode.MALA ? currentTheme.card : 'transparent',
              color: mode === CountingMode.MALA ? currentTheme.primary : currentTheme.text
            }}
          >
            माला (108)
          </button>
          <button 
            ...
            className={`px-6 py-2 rounded-xl text-xs font-bold transition-all ${mode === CountingMode.UNLIMITED ? 'shadow-sm' : 'opacity-40'}`}
            style={{ 
              backgroundColor: mode === CountingMode.UNLIMITED ? currentTheme.card : 'transparent',
              color: mode === CountingMode.UNLIMITED ? currentTheme.primary : currentTheme.text
            }}
          >
            असीमित
          </button>
      ...
      <main className="flex-1 w-full max-w-md flex flex-col items-center justify-center gap-6 py-4">
        ...
        <div className="relative group my-4">
          <button
            ...
            className={`
              w-64 h-64 md:w-72 md:h-72 rounded-full shadow-2xl flex flex-col items-center justify-center
              transition-all duration-150 active:scale-90 relative overflow-hidden z-10
          ...
          >
            ...
            <div className="absolute inset-0 bg-white opacity-0 active:opacity-20 transition-opacity"></div>
          </button>
          
          {/* Subtle Outer Glow */}
          <div 
            className="absolute inset-0 rounded-full blur-3xl opacity-20 -z-10 animate-pulse"
            style={{ backgroundColor: currentTheme.primary }}
          ></div>
        </div>

        {/* Mala Progress - Only visible in MALA mode */}
        {mode === CountingMode.MALA && (
          <div className="w-full bg-white/40 rounded-3xl p-6 shadow-sm backdrop-blur-md transform transition-all animate-in fade-in slide-in-from-bottom-4">
            ...
            <div className="w-full h-3 bg-gray-200/50 rounded-full overflow-hidden">
              <div 
                className="h-full transition-all duration-300 ease-out rounded-full shadow-inner"
                style={{ width: `${progressPercentage}%`, backgroundColor: currentTheme.primary }}
              ></div>
            </div>
          </div>
        )}
      ...
      {isResetModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-white rounded-3xl p-8 max-w-xs w-full shadow-2xl transform transition-all animate-in fade-in zoom-in duration-200">
            ...
            <div className="flex gap-3">
              <button 
                ...
                className="flex-1 py-3 px-4 rounded-xl font-bold text-gray-700 bg-gray-100 active:bg-gray-200 transition-colors"
              >
                रद्द करें
              </button>
              <button 
                onClick={handleReset}
                className="flex-1 py-3 px-4 rounded-xl font-bold text-white bg-red-500 active:bg-red-600 shadow-lg shadow-red-200 transition-all"
              >
                रीसेट
              </button>
            </div>
